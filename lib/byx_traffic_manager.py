...
class ByxTrafficManager:
    ...
    def __init__(self, param):
        ...
        self.dnsPort = BymUtil.getFreeSocketPort("tcp")
    ...
    def on_wan_conn_up(self):
        ...
        # BymUtil.shell('/sbin/nft add rule wrtd fw iifname %s ct state established,related accept' % (intf))
        # BymUtil.shell('/sbin/nft add rule wrtd fw iifname %s ip protocol icmp accept' % (intf))
        # BymUtil.shell('/sbin/nft add rule wrtd fw iifname %s drop' % (intf))
    ...
    def _stopDnsmasq(self):
        if self.dnsmasqProc is not None:
            ...
            self.dnsmasqProc = None
        BymUtil.forceDelete(self.hostsDir)
        BymUtil.forceDelete(self.pidFile)
        BymUtil.forceDelete(self.cfgFile)
...
class _Helper:
    ...
    def prefixConvert(prefix):
        tl = prefix.split("/")
        return tl[0] + "/" + str(BymUtil.ipMaskToLen(tl[1]))
